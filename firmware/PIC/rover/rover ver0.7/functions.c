...
char *readSwitch(void) {
    ...
    LATDbits.LATD6 = 0; //DATA PIN
    LATDbits.LATD5 = 0; //CLOCK PIN
    LATDbits.LATD7 = 0; //STORAGE PIN
    ...
    for (int i = 0; i < 8; i++) {
        if (i == 0) LATDbits.LATD6 = 1; //DATA PIN
        else LATDbits.LATD6 = 0; //DATA PIN
        LATDbits.LATD5 = 1;
        __delay_us(100);
        ...
        __delay_ms(10);
        LATDbits.LATD7 = 0;
        if(PORTCbits.RC7 == 0) array[i] = 'L';
        else array[i] = 'H';
    }

    LATDbits.LATD6 = 1; //DATA PIN
